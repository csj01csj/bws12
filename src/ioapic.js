...
function IOAPIC(cpu)
{
    ...
    cpu.io.mmap_register(IOAPIC_ADDRESS, MMAP_BLOCK_SIZE,
        ...
        {
            addr = addr - IOAPIC_ADDRESS | 0;
            if(addr === IOREGSEL) return this.ioregsel;
            else if(addr === IOWIN) return this.read(this.ioregsel);
            else { dbg_assert(false); return 0; }
        },
        (addr, value) =>
        {
            addr = addr - IOAPIC_ADDRESS | 0;
            if(addr === IOREGSEL) this.ioregsel = value;
            else if(addr === IOWIN) this.write(this.ioregsel, value);
...
IOAPIC.prototype.check_irq = function(irq)
{
    ...
    if((config & IOAPIC_CONFIG_MASKED) === 0)
    {
        ...
        else
        {
            ...
            if(config & IOAPIC_CONFIG_REMOTE_IRR) return;
        }
...
{
    if(i >= IOAPIC_IRQ_COUNT) { dbg_assert(false, "Bad irq: " + i, LOG_APIC); return; }
...
IOAPIC.prototype.clear_irq = function(i)
{
    ...
    if((this.irq_value & mask) === mask)
    {
        this.irq_value &= ~mask;
        var config = this.ioredtbl_config[i];
        if(config & IOAPIC_CONFIG_TRIGGER_MODE_LEVEL) this.irr &= ~mask;
    }
};

IOAPIC.prototype.read = function(reg)
{
    if(reg === 0) return this.ioapic_id << 24;
    else if(reg === 1) return 0x11 | IOAPIC_IRQ_COUNT - 1 << 16;
    else if(reg === 2) return this.ioapic_id << 24;
    else if(reg >= 0x10 && reg < 0x10 + 2 * IOAPIC_IRQ_COUNT)
    {
        var irq = reg - 0x10 >> 1;
        var index = reg & 1;
        var value = index ? this.ioredtbl_destination[irq] : this.ioredtbl_config[irq];
        return value;
    }
    else { dbg_assert(false); return 0; }
};
...
{
    if(reg === 0) this.ioapic_id = value >>> 24 & 0x0F;
    else if(reg === 1 || reg === 2) dbg_log("Invalid write: " + reg, LOG_APIC);
    else if(reg >= 0x10 && reg < 0x10 + 2 * IOAPIC_IRQ_COUNT)
    {
        var irq = reg - 0x10 >> 1;
        var index = reg & 1;
