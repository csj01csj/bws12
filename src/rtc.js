...
RTC.prototype.bcd_pack = function(n)
{
    ...
    while(n) { digit = n % 10; result |= digit << (4 * i); i++; n = (n - digit) / 10; }
    return result;
};
...
{
    if(this.cmos_b & 4) return t;
    else return this.bcd_pack(t);
};

RTC.prototype.decode_time = function(t)
{
    if(this.cmos_b & 4) return t;
    else return this.bcd_unpack(t);
};
...
RTC.prototype.cmos_port_read = function()
{
    ...
    switch(index)
    {
        case CMOS_RTC_SECONDS: return this.encode_time(new Date(this.rtc_time).getUTCSeconds());
        case CMOS_RTC_MINUTES: return this.encode_time(new Date(this.rtc_time).getUTCMinutes());
        case CMOS_RTC_HOURS: return this.encode_time(new Date(this.rtc_time).getUTCHours());
        case CMOS_RTC_DAY_MONTH: return this.encode_time(new Date(this.rtc_time).getUTCDate());
        case CMOS_RTC_MONTH: return this.encode_time(new Date(this.rtc_time).getUTCMonth() + 1);
        case CMOS_RTC_YEAR: return this.encode_time(new Date(this.rtc_time).getUTCFullYear() % 100);
        case CMOS_STATUS_A:
            if(v86.microtick() % 1000 >= 999) return this.cmos_a | 0x80;
            return this.cmos_a;
        case CMOS_STATUS_B: return this.cmos_b;
        case CMOS_STATUS_C:
            ...
            this.cmos_c &= ~0xF0;
            return c;
        case CMOS_STATUS_D: return 0;
        case CMOS_CENTURY: return this.encode_time(new Date(this.rtc_time).getUTCFullYear() / 100 | 0);
        default:
...
RTC.prototype.cmos_port_write = function(data_byte)
{
    switch(this.cmos_index)
    {
        ...
        case 0xB:
            ...
            if(this.cmos_b & 0x40) this.next_interrupt = Date.now();
            if(this.cmos_b & 0x20)
            {
                const now = new Date();
                ...
                const alarm_date = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), hours, minutes, seconds));
                const ms_from_now = alarm_date - now;
                dbg_log("RTC alarm scheduled for " + alarm_date + " ms_from_now=" + ms_from_now, LOG_RTC);
